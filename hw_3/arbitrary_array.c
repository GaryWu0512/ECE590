...
char * ArbitraryArray_to_string(const ArbitraryArray * a, char * (*element_to_string) (void *) ){
    ...
    int nm = ArbitraryArray_size(a);
    char * s = (char *)calloc(40, sizeof(char));
    int i,j=0;
    for (i = 0; i<nm; i++){
        if ( i < nm - 1 ) {
            sprintf( s + j, "%s,", element_to_string(ArbitraryArray_get_ptr( a, i)));
            j += strlen(element_to_string(ArbitraryArray_get_ptr( a, i))) + 1;
        } else {
            sprintf( s + j, "%s", element_to_string(ArbitraryArray_get_ptr( a, i)));
            j += strlen(element_to_string(ArbitraryArray_get_ptr( a, i)));
        }
    }
    return s;
}