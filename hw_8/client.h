...
namespace elma {
    ...
    class Client {
        ...
        void _get_thread_post(std::string url, json data,  std::function<void(json&)> handler);
        const std::shared_ptr<httplib::Response> _get_aux(std::string url);
        const std::shared_ptr<httplib::Response> _post_aux(std::string url, std::string data);
        //const std::shared_ptr<httplib::Response> _get_aux_post(std::string url, string data);
