...
namespace elma {
    ...
    Client& Client::post(std::string url, json data, std::function<void(json&)> handler) {
        // TODO: problem 2
        std::thread t(&Client::_get_thread_post,this,url,data, handler);
        t.detach(); // detaching means we don't have to join later
    ...
    const std::shared_ptr<httplib::Response> Client::_post_aux(std::string url, std::string data) {
        auto parts = url_parts(url);
        if ( _use_ssl ) {
            httplib::SSLClient cli(parts.first.c_str(), 443);
            return cli.Post(parts.second.c_str(), data, "json");              
        } else {
            httplib::Client cli(parts.first.c_str(), 80);
            return cli.Post(parts.second.c_str(), data, "json");
        }
    ...
    void Client::_get_thread_post(std::string url, json data,  std::function<void(json&)> handler) {

        json json_response;

        try {
            auto response = _post_aux(url, data.dump());
